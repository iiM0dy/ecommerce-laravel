...
@section('content')
    <div class="container">
        <h2>{{ __('messages.add_product_photo') }}</h2>
        ...
        <form action="{{ url('/storeproductphoto') }}" enctype="multipart/form-data" method="post" class="photo-form">
            ...
            <label for="photo">{{ __('messages.choose_image') }}</label>
            <input type="file" name="image" id="image">


            <button type="submit">{{ __('messages.add_photo') }}</button>
        </form>
            ...
            @foreach ($productImages as $photo)
                <div class="gallery-item">
                    ...
                    <a href="/deleteproductphoto/{{ $photo->id }}" class="delete-btn"
                        onclick="return confirm('{{ __('messages.are_you_sure_you_want_to_delete_this_image') }}')">{{ __('messages.delete') }}</a>
                </div>
            @endforeach
        </div>
    </div>
